...
class AggressionModeTests(unittest.TestCase):
    def test_offensive_mode_lowers_entry_thresholds_and_cooldown(self) -> None:
        with patch.dict("os.environ", {"AGGRESSION_MODE": "OFFENSIVE"}, clear=False):
            self.assertEqual(float(get_runtime_setting("ADVISORY_MIN_ENTRY_SCORE")), 57.0)
            self.assertEqual(int(get_runtime_setting("TRADER_COOLDOWN_BARS")), 2)

    def test_defensive_mode_raises_thresholds_and_reduces_weight(self) -> None:
        with patch.dict("os.environ", {"AGGRESSION_MODE": "DEFENSIVE"}, clear=False):
            ...
            self.assertAlmostEqual(float(get_runtime_setting("TRADER_PROPOSED_WEIGHT")), 0.09, places=4)

    def test_snapshot_exposes_base_and_adjusted_values(self) -> None:
        ...
        self.assertEqual(snapshot["base_values"]["ADVISORY_MIN_ENTRY_SCORE"], 60.0)
        self.assertEqual(snapshot["values"]["ADVISORY_MIN_ENTRY_SCORE"], 55.0)
